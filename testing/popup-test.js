...
const devNg =
    'https://micamdevw.michigan.gov/tpaas/ted/module/contract/invoice/list';
const prodNg =
    'https://miloginworker.michigan.gov/tpaas/ted/module/contract/invoice/list';

const dev = 'https://micamdevw.michigan.gov/tpaas/ted/electronicpayer/list';
const prod =
    'https://miloginworker.michigan.gov/tpaas/ted/electronicpayer/list';
...
test('Switch envs and strip uuid devNg to prodNg keeping query params', () => {
  assert.equal(m.Main.replaceBaseUrl(devNg + uuid + qParams, 'prod'),
               prodNg + qParams);
});

test('Same env dont strip uuid devNg to localNg keeping query params', () => {
  assert.equal(m.Main.replaceBaseUrl(devNg + uuid + qParams, 'local4200'),
               localNg4200 + uuid + qParams);
});
