...
this.Main = Object.freeze({init, replaceBaseUrl});

async function init(envButtons) {
  const tabs = await browser.tabs.query({active : true, currentWindow : true});
  const activeTab = tabs[0];
  const isTedUrl = Object.values(ENV_URLS).findIndex(
                       (url) => { return activeTab.url.startsWith(url); },
                       activeTab.url) > -1;
  // Display text based on what the current url is
  const text = isTedUrl ? "Open this page in what environment?"
...
function replaceBaseUrl(url, targetId) {
  ...
  let fromEnv =
      Object.keys(ENV_URLS).find(key => url.indexOf(ENV_URLS[key]) > -1)
  ...
  url = url.replace(ENV_URLS[fromEnv], '')
            .replace(TED, '')
            .replace(MODULE, '')
            .replace(LOCAL, '');
  while (url.charAt(0) == '/') {
...
function bug(newBug) { bugs.push(newBug); }
}.call(this));
